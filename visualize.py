...
def main():
    parser = build_argparser("visualize")
    ...
    pygame.init()
    ecran = pygame.display.set_mode((cfg.largeur_ecran, cfg.hauteur_ecran), pygame.RESIZABLE)
    ...
    os.makedirs(cfg.dossier_videos, exist_ok=True)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    chemin_video = os.path.join(cfg.dossier_videos, cfg.nom_video)
    video_writer = cv2.VideoWriter(chemin_video, fourcc, cfg.fps_video, (largeur, hauteur))

    with torch.no_grad():
        for frame in range(cfg.nb_frames_visualisation):
            ...
            if frame % 5 == 0:
                ...
                action = action + torch.randn_like(action) * cfg.bruit_visualisation       # reforme pour apply_action
    ...
    print(f"🎬 Vidéo de démonstration sauvegardée avec succès dans {chemin_video} !")
